...
const IMMEDIATE_FIX_PERCENTAGE = 75;
// % of mistakes fixed immediately
...
let advancedSettings = {
    immediateFixPercentage: IMMEDIATE_FIX_PERCENTAGE,
    minCharsBeforeReview: MIN_CHARS_BEFORE_REVIEW,
    maxCharsBeforeReview: MAX_CHARS_BEFORE_REVIEW,
    minReviewPause: MIN_REVIEW_PAUSE_MS,
    maxReviewPause: MAX_REVIEW_PAUSE_MS,
    minImmediateFixPause: MIN_IMMEDIATE_FIX_PAUSE_MS,
    maxImmediateFixPause: MAX_IMMEDIATE_FIX_PAUSE_MS,
    minBetweenFixPause: MIN_BETWEEN_FIX_PAUSE_MS,
    maxBetweenFixPause: MAX_BETWEEN_FIX_PAUSE_MS,
    allowSelection: true,
    allowWordNavigation: true
};
// Load settings from chrome.storage.local when script initializes
chrome.storage.local.get([
    'speedFactor', 'randomnessFactor', 'mistakeProbability',
    'immediateFixPercentage', 'minCharsBeforeReview', 'maxCharsBeforeReview',
    'minReviewPause', 'maxReviewPause', 'minImmediateFixPause', 'maxImmediateFixPause',
    'minBetweenFixPause', 'maxBetweenFixPause', 'allowSelection', 'allowWordNavigation'
], function(result) {
    ...
    if (result.immediateFixPercentage !== undefined) {
        advancedSettings.immediateFixPercentage = result.immediateFixPercentage;
    }
    if (result.minCharsBeforeReview !== undefined) {
        advancedSettings.minCharsBeforeReview = result.minCharsBeforeReview;
}
    if (result.maxCharsBeforeReview !== undefined) {
        advancedSettings.maxCharsBeforeReview = result.maxCharsBeforeReview;
}
    if (result.minReviewPause !== undefined) {
        advancedSettings.minReviewPause = result.minReviewPause;
}
    if (result.maxReviewPause !== undefined) {
        advancedSettings.maxReviewPause = result.maxReviewPause;
}
    if (result.minImmediateFixPause !== undefined) {
        advancedSettings.minImmediateFixPause = result.minImmediateFixPause;
}
    if (result.maxImmediateFixPause !== undefined) {
        advancedSettings.maxImmediateFixPause = result.maxImmediateFixPause;
}
    if (result.minBetweenFixPause !== undefined) {
        advancedSettings.minBetweenFixPause = result.minBetweenFixPause;
}
    if (result.maxBetweenFixPause !== undefined) {
        advancedSettings.maxBetweenFixPause = result.maxBetweenFixPause;
}
...
let i = 0;
    ...
    let reviewCharThreshold = getRandomInt(advancedSettings.minCharsBeforeReview, advancedSettings.maxCharsBeforeReview);
let mistakeLog = [];
    let fieldPos = getCaretPosition(activeElement);
    let fieldStart = getTextLength(activeElement);
// Track where this typing session starts
    typingState = {
        isPaused: false,
        session,
        text,
...
state.mistakeLog.push({absPos: state.fieldStart + state.fieldPos, wrong: wrongChar, correct: state.text[state.i]});
            console.log(`[DEBUG] Mistake logged: "${wrongChar}" -> "${state.text[state.i]}" at pos ${state.fieldStart + state.fieldPos}. Total mistakes: ${state.mistakeLog.length}`);
...
setTimeout(() => proceedTyping(), getRandomInt(advancedSettings.minImmediateFixPause, advancedSettings.maxImmediateFixPause));
                return;
            } else {
                state.i++;
...
setTimeout(() => proceedTyping(), getRandomInt(advancedSettings.minReviewPause, advancedSettings.maxReviewPause));
            return;
        }
                let delay;
...
return;
    ...
    const immediateFix = state.immediateFix;
    if (immediateFix.step === 0) {
        stealthyBackspace(state.activeElement);
adjustMistakeLogPositions(state.mistakeLog, state.fieldStart + state.fieldPos);
        logDebugState('After backspace (immediate fix)', state);
        immediateFix.step = 1;
        setTimeout(() => proceedTyping(), getRandomInt(advancedSettings.minImmediateFixPause, advancedSettings.maxImmediateFixPause));
} else if (immediateFix.step === 1) {
        stealthyInsertText(state.activeElement, state.text[state.i]);
logDebugState('After correct insert (immediate fix)', state);
        state.i++;
        state.charsSinceLastReview++;
        state.phase = 'typing';
        state.immediateFix = null;
...
return;
    ...
    let indices = review.order === 'forward'
        ?
[...Array(state.mistakeLog.length).keys()].filter(i => !state.mistakeLog[i].fixed)
        : [...Array(state.mistakeLog.length).keys()].filter(i => !state.mistakeLog[i].fixed).reverse();
if (review.fixIndex >= indices.length) {
        // Review complete for this batch
        logDebugState('After review phase complete', state);
// Filter out fixed mistakes from the log
        state.mistakeLog = state.mistakeLog.filter(m => !m.fixed);
// Check if this was the final review (end of typing)
        if (state.i >= state.text.length) {
            // Only clear mistake log if we're at the end of typing
            console.log(`[DEBUG] Clearing mistake log at end of typing. Mistakes cleared: ${state.mistakeLog.length}`);
state.mistakeLog = [];
            state.review = null;
            clearTypingState();
            return;
        } else {
            // Continue typing, but keep the mistake log for future reviews
            state.phase = 'typing';
state.charsSinceLastReview = 0;
            state.review = null; // Clear the review object
            state.reviewCharThreshold = getRandomInt(advancedSettings.minCharsBeforeReview, advancedSettings.maxCharsBeforeReview);
setTimeout(() => proceedTyping(), state.baseDelay);
        }
        return;
}
    
    // Track step: 0 = navigate to position, 1 = backspace, 2 = insert correct
    if (!('step' in review)) review.step = 0;
const idx = indices[review.fixIndex];
    const {absPos, wrong, correct} = state.mistakeLog[idx];
if (review.step === 0) {
        // Navigate to the position using arrow keys
        const currentPos = getCaretPosition(state.activeElement);
navigateWithArrowKeys(
            state.activeElement, 
            absPos + 1, 
            currentPos, 
            advancedSettings.allowSelection, 
            advancedSettings.allowWordNavigation
        );
review.step = 1;
        setTimeout(() => proceedTyping(), getRandomInt(advancedSettings.minBetweenFixPause, advancedSettings.maxBetweenFixPause));
    } else if (review.step === 1) {
        // Use backspace to remove the wrong character
        stealthyBackspace(state.activeElement);
adjustMistakeLogPositions(state.mistakeLog, absPos);
        ...
        // Navigate to correct position and insert
        const currentPos = getCaretPosition(state.activeElement);
        navigateWithArrowKeys(state.activeElement, absPos, currentPos, false, false);
        stealthyInsertText(state.activeElement, correct);
        logDebugState('After correct insert (review fix)', state);
        
        // Mark the mistake as fixed
        state.mistakeLog[idx].fixed = true;
review.fixIndex++;
        review.step = 0;
        setTimeout(() => proceedTyping(), 0);
    }
